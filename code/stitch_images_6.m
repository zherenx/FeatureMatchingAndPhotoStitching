function im = stitch_images_6(sImage, cImage, H)
...
im_background = ims .* mask3d;

ims_blend = poissonBlend(ims,~mask3d,im_background);
imc_blend = poissonBlend(imc,~mask3d,im_background);

ims_blend = ims_blend .* (ims_blend & ims);
imc_blend = imc_blend .* (imc_blend & imc);

% im = ims_blend + imc_blend .* ~overlap;

im = ims_blend + imc_blend;

im = im .* (im & overlap) / 2 + im .* (im & ~overlap);
